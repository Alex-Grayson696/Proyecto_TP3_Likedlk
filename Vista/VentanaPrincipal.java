...
public class VentanaPrincipal extends JFrame {
    ...
    private JComboBox<String> comboSugerencia;   // Pestaña Sugerencias
    private JTextArea         areaResultado;
    private JTextArea         areaSugerencias;  // Pestaña Sugerencias
    ...
    private void construirUI() {
        ...
        pestanas.addTab("💡  Sugerencias",     crearPestanaSugerencias());
    ...
    // ─── PESTAÑA 5: Sugerencias de Networking ───────────────────────────────
    
    private JPanel crearPestanaSugerencias() {
        JPanel panel = new JPanel(new BorderLayout(8, 8));
        panel.setBackground(GRIS_FONDO);
        panel.setBorder(new EmptyBorder(10, 10, 10, 10));
        ...
        controles.setBackground(BLANCO);
        controles.setBorder(new CompoundBorder(
            new TitledBorder(new LineBorder(new Color(106, 27, 154)), " Generar Sugerencias "),
        ...
        comboSugerencia = new JComboBox<>();
        comboSugerencia.setFont(FUENTE_NORMAL);

        gbc.gridx=0; gbc.gridy=0; gbc.weightx=0.25;
        controles.add(etiqueta("Seleccionar nodo:"), gbc);
        gbc.gridx=1; gbc.weightx=0.75;
        controles.add(comboSugerencia, gbc);

        // Botón principal
        JButton btnSugerir = boton("💡 Generar Sugerencias de Estudiantes", new Color(106, 27, 154));
        btnSugerir.addActionListener(e -> {
            String id = idDesdeCombo(comboSugerencia);
            areaSugerencias.setText(controlador.ejecutarSugerencias(id));
        });

        gbc.gridx=0; gbc.gridy=1; gbc.gridwidth=2;
        controles.add(btnSugerir, gbc);

        // ── Área de resultados ──
        areaSugerencias = new JTextArea();
        areaSugerencias.setFont(FUENTE_MONO);
        areaSugerencias.setEditable(false);
        areaSugerencias.setBackground(new Color(30, 30, 30));
        areaSugerencias.setForeground(new Color(255, 213, 79));  // Amarillo dorado
        areaSugerencias.setBorder(new EmptyBorder(10, 12, 10, 12));
        areaSugerencias.setText("Seleccione un nodo y pulse el botón para ver estudiantes sugeridos...");

        JScrollPane scrollResultado = new JScrollPane(areaSugerencias);
        scrollResultado.setBorder(new TitledBorder(
            new LineBorder(new Color(50, 50, 50), 1), " Estudiantes Sugeridos "));

        // ── Panel informativo ──
        JPanel infoPanel = tarjetaInfo(
            "¿Cómo funciona este algoritmo?",
            "Usa BFS de 2 saltos: busca todos los nodos a distancia 2 del seleccionado " +
            "y filtra solo los de tipo ESTUDIANTE. Excluye los que ya están conectados " +
            "directamente. Reproduce el mecanismo \"Personas que quizás conozcas\" de LinkedIn.",
            new Color(237, 231, 246), new Color(106, 27, 154));
    ...
    private void actualizarCombos() {
        ...
        // Actualizar el combo de la pestaña Sugerencias
        if (comboSugerencia != null) {
            DefaultComboBoxModel<String> mSug = new DefaultComboBoxModel<>();
            for (Nodo n : controlador.getNodos()) {
                mSug.addElement(n.getNombre() + " [" + n.getId() + "]");
            }
            comboSugerencia.setModel(mSug);
        }
    }
    ...
    /** Igual que idDesdeCombo pero para los combos de conexiones. */
